...
export interface VizSettings {
  ...
  coreSize: number;     // 0.5 to 2.0 (Acts as Scale now since core is removed)
  
  // Background
  showHexagons: boolean;
  hexOpacity: number;   // 0 to 1
  hexVisualMode: 'pulse' | 'spectrum' | 'wave'; // New Visual Modes
  
  // Water Effect
  showWaterRipples: boolean; // New Ripple Effect
  hydroIntensity: number; // 0.1 to 2.0
  
  // Overlays
  showTreeOfLife: boolean; // Sacred Geometry Overlay
  ...
  morphEnabled: boolean; // Toggle for the breathing cloud/geometry animation
}